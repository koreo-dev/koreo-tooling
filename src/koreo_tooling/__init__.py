...
def get_crd_path():
    ...
    try:
        ...
        # Fallback to local CRDs for development (look in parent directories)
        current_dir = pathlib.Path(__file__).parent
        
        # Try relative to tooling project root
        dev_crd = current_dir.parent.parent.parent / "crd"
        if dev_crd.exists():
...
def load_schema_validators():
    ...
    try:
        ...
        # This will use the core package's CRD_ROOT automatically
        from koreo.schema import load_validators_from_files
        load_validators_from_files(clear_existing=True)
        
        logger.info("Schema validators loaded from koreo-core package")
